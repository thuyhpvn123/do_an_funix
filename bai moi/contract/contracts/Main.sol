...
contract Main {
    ...
    function update(uint id, string memory _newName, string memory _newEmail) public{
        // for(uint i;i<count;i++){
        //     if(Iparticipants[i].participantID == id){
    ...
    function statusSession(address _session,uint _status) public onlyAdmin{
    ...
    function closeSession(address _session)public onlyAdmin returns(uint){
    ...
    function getProfile(uint _id) public returns(uint ,address,string memory,string memory){
        return(Iparticipants[_id].participantID,Iparticipants[_id].addr,Iparticipants[_id].name,Iparticipants[_id].email);
    }
